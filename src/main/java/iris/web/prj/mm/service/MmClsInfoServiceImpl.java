...
public class MmClsInfoServiceImpl implements MmClsInfoService {
     ...
     */
        ...
        public void updateMmIlckSap(List<Map<String, Object>> dataSetList, HashMap<String, Object> input) throws JCoException{
            ...
            LOGGER.debug("#################################### call "+ functionName +" START ####################################");
            ...
            try{ //연결정보확인.
                ...
                LOGGER.debug("SAP연동 실행완료::!!");

                updateMmClsIlck(list);

            }catch(Exception e){
                e.printStackTrace();
                LOGGER.error("============SAP연동중 Error가 발생 ::!! : " + e);
                throw new RuntimeException("============SAP연동중 Error가 발생 ::!! : ", e);
            } finally {
                LOGGER.debug("#################################### call "+ functionName +" END ####################################");
            }
        }
