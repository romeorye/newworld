...
public class MarkAnyDrm {
	...
	//static final String sDrmServerIp = "165.244.161.87";
	static final String sDrmServerIp = "10.39.178.219";
	...
	static final Logger LOGGER = LogManager.getLogger(MarkAnyDrm.class);
	...
	public int fncEncyto(Map<String, Object> attachFileInfo, HashMap<String, Object> input, Map<String, Object> drmCfgMap) {
		...
		m_enc_dec.sDestFilePath =  "D:\\encode\\"+attachFileInfo.get("filPath").toString().substring(attachFileInfo.get("filPath").toString().lastIndexOf("\\") + 1, attachFileInfo.get("filPath").toString().length());			//파일을 포함하는 풀 경로(절대 경로)
