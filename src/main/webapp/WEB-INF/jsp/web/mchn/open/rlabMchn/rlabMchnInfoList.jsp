 ...
 *************************************************************************
 * $Id		: mchnInfoList.jsp
 * @desc    : Instrument > 신뢰성시험 장비 > 보유 장비 > 보유장비 리스트
 *------------------------------------------------------------------------
...
<script type="text/javascript" src="<%=ruiPathPlugins%>/tab/rui_tab.js"></script>
	...
	<script type="text/javascript">
		...
		Rui.onReady(function() {
			...
			var tabView = new Rui.ui.tab.LTabView({
                tabs: [
                    ...
                    { label: '옥산 신뢰성시험',       	content: '<div id="tabContent1"></div>' },
            		{ label: '내후성 시험',        content: '<div id="tabContent2"></div>' },
                    { label: '측정, 평가',        content: '<div id="tabContent3"></div>' }
                ]
            });
			...
			goPage = function(target, mchnClCd) {
	        	...
	        	$('#opnYn').val("ALL");
	        	tabUrl = "<c:url value='/mchn/open/rlabMchn/retrieveMchnTab.do'/>";
		...
		function fncMchnDtlMove(id){
			...
			frm.target = "_self";
			frm.action = "<c:url value="/mchn/open/rlabMchn/retrieveMchnDtl.do"/>";
			frm.submit();
		}
