...
var mchnPrctId;
	...
	function fncPrevMon(){
		...
		frm.rtnUrl.value ="web/mchn/open/mchn/mchnPrctTab";
		frm.action = '<c:url value='/mchn/open/mchn/retrieveMchnPrctTab.do'/>';
		frm.submit();
	}
...
<div>
	...
	<div class="sub-content">
        ...
        <div class="sch_reserv_present">
            <font><b>예약방법</b> : 기기에 대한 예약을 원하시면 해당 날짜의 예약 현황을 확인 하신 후  일자를 클릭 하시면 예약이  가능 합니다.</font>
          	...
          	<ul class='sche'>
          		...
          		if ( mchnPrctList.size() > 0 ){
          			for (int z=0; z < mchnPrctList.size(); z++){
          				...
          				if( dispDay == chkDt ){
          		%>
          		<li><a href="javascript:parent.fncMchnPrctPop('<%=item.get("mchnPrctId")%>','<%=year%>', '<%=month%>','<%=dispDay%>');"><%=item.get("prctTim")%>&nbsp;<span><%=item.get("rgstNm") %></span>&nbsp;<%=item.get("prctScnNm") %> </a></li> 
          		<%
          				}
          			}
          		}
