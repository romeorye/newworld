	...
	Rui.onReady(function() {
           ...
           rgstNm.on('keypress', function(e) {
			...
			var swrmStrtDt = new Rui.ui.form.LDateBox({
				...
				mask: '9999-99-99',
				displayValue: '%Y-%m-%d',
				defaultValue: '<c:out value="${inputData.swrmStrtDt}"/>',
...
// 				}
			...
			var swrmFnhDt = new Rui.ui.form.LDateBox({
				...
				mask: '9999-99-99',
				displayValue: '%Y-%m-%d',
				defaultValue: '<c:out value="${inputData.swrmFnhDt}"/>',
				//defaultValue: new Date(),
				width: 100,
				dateType: 'string'
			});
...
function fncExcelDown() {
    ...
    if( showDataSet.getCount() > 0){
    	...
    	nG.saveExcel(toUTF8('전시회 목록_') + new Date().format('%Y%m%d') + '.xls');
    } else {
    	alert('조회된 데이타가 없습니다.!!');
    }
...
init = function() {
	   ...
	   showDataSet.load({
         ...
         params :{
         	...
         	swrmNatCd : '${inputData.swrmNatCd}',
         	swrmStrtDt : '${inputData.swrmStrtDt}',
         	swrmFnhDt : '${inputData.swrmFnhDt}'
         }
     });
 }
</script>
    ...
    <body onkeypress="if(event.keyCode==13) {getShowList();}" onload="init();">
