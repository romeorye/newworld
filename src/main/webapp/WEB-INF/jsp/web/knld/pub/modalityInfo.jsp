...
<link rel="stylesheet" type="text/css" href="<%=ruiPathPlugins%>/ui/grid/LGridStatusBar.css"/>
	...
	var roleIdIndex = roleId.indexOf("WORK_IRI_T01");
		...
		Rui.onReady(function() {
             ...
             *******************/
            <%-- DATASET --%>
            modalityInfoDataSet = new Rui.data.LJsonDataSet({
                id: 'modalityInfoDataSet',
                remainRemoved: true,
                lazyLoad: true,
                fields: [
            ...
            modalityInfoDataSet.on('load', function(e) {
//             	var sbcNm = modalityInfoDataSet.getNameValue(0, "sbcNm").replaceAll('\n', '<br/>');
//             	modalityInfoDataSet.setNameValue(0, 'sbcNm', sbcNm);

                lvAttcFilId = modalityInfoDataSet.getNameValue(0, "attcFilId");
                if(!Rui.isEmpty(lvAttcFilId)) getAttachFileList();

                //등록자와 사용자가 다를때 수정/삭제버튼 가리기
                pRgstId = modalityInfoDataSet.getNameValue(0, "rgstId");
            ...
            /* [DataSet] bind */
            var modalityInfoBind = new Rui.data.LBind({
                groupId: 'aform',
                dataSet: modalityInfoDataSet,
                bind: true,
                bindInfo: [
                    { id: 'modalityId',     ctrlId: 'modalityId',      value: 'value'} //표준양식ID
            ...
            setAttachFileInfo = function(attachFileList) {
                ...
                if(Rui.isEmpty(lvAttcFilId)) {
                	...
                	modalityInfoDataSet.setNameValue(0, "attcFilId", attachFileList[0].data.attcFilId);
                }
            };
            ...
            modalityRgstSave = function() {
         	   var record = modalityInfoDataSet.getAt(modalityInfoDataSet.getRow());
            ...
            /* [버튼] 삭제 */
           modalityRgstDel = function() {
                fncDeleteModalityInfo();
            };

    		/* [버튼] 목록 */
            goModalityList = function() {
            	//$(location).attr('href', '<c:url value="/knld/pub/retrieveModalityList.do"/>');
            	nwinsActSubmit(document.searchForm, "<c:url value='/knld/pub/retrieveModalityList.do'/>");
            };
	...
	<%--/*******************************************************************************
   	 * FUNCTION 명 : fncDeleteModalityInfo (표준양식 삭제)
   	 * FUNCTION 기능설명 : 표준양식 삭제
   	 *******************************************************************************/--%>
	    fncDeleteModalityInfo = function(){

	    	var dm1 = new Rui.data.LDataSetManager({defaultFailureHandler: false});

    		dm1.updateDataSet({
    	        url: "<c:url value='/knld/pub/deleteModalityInfo.do'/>",
    	        dataSets:[modalityInfoDataSet],
    	        params: {
    	        	modalityId : modalityId
    	        }
	    	});
	    	...
	    	dm1.on('success', function(e) {
				var resultData = modalityInfoDataSet.getReadData(e);
