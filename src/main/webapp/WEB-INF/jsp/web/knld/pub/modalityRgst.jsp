...
//             });
            ...
            <%-- DATASET --%>
            modalityRgstDataSet = new Rui.data.LJsonDataSet({
                id: 'modalityRgstDataSet',
                remainRemoved: true,
                lazyLoad: true,
                fields: [
				      { id: 'modalityId' }         /*표준양식ID*/
					, { id: 'titlNm' }           /*제목*/
					, { id: 'sbcNm' }            /*본문내용*/
					, { id: 'rtrvCnt' }          /*조회수*/
					, { id: 'keywordNm' }        /*키워드*/
            ...
            modalityRgstDataSet.on('load', function(e) {
            	lvAttcFilId = modalityRgstDataSet.getNameValue(0, "attcFilId");
                if(!Rui.isEmpty(lvAttcFilId)) getAttachFileList();

//                 var sbcNm = modalityRgstDataSet.getNameValue(0, "sbcNm").replaceAll('\n', '<br/>');
//                 modalityRgstDataSet.setNameValue(0, 'sbcNm', sbcNm);

                if(modalityRgstDataSet.getNameValue(0, "modalityId")  != "" ||  modalityRgstDataSet.getNameValue(0, "modalityId")  !=  undefined ){
    				document.aform.Wec.value=modalityRgstDataSet.getNameValue(0, "sbcNm");
    			}
            });

            /* [DataSet] bind */
            var modalityRgstBind = new Rui.data.LBind({
                groupId: 'aform',
                dataSet: modalityRgstDataSet,
                bind: true,
                bindInfo: [
                      { id: 'modalityId',     ctrlId: 'modalityId',      value: 'value' }
            ...
            setAttachFileInfo = function(attachFileList) {
                ...
                if(Rui.isEmpty(lvAttcFilId)) {
                	...
                	modalityRgstDataSet.setNameValue(0, "attcFilId", attachFileList[0].data.attcFilId);
                }
            };
            ...
            /* [버튼] 저장 */
            modalityRgstSave = function() {
                fncInsertModalityInfo();
            };

    		/* [버튼] 목록 */
            goModalityList = function() {
            	$(location).attr('href', '<c:url value="/knld/pub/retrieveModalityList.do"/>');
            };
...
// 		    	}
        ...
        vm = new Rui.validate.LValidatorManager({
            validators:[
            	{ id: 'titlNm',      validExp: '양식명:true:maxByteLength=400' },
	   ...
	   function validation(vForm){
		...
		fn_init = function(){
			...
			var modalityId = '${inputData.modalityId}';

	    	if(pageMode == 'V'){

	             /* 상세내역 가져오기 */
	             getModalityInfo = function() {
	            	 modalityRgstDataSet.load({
	                     url: '<c:url value="/knld/pub/getModalityInfo.do"/>',
	                     params :{
	    		...
	    		modalityRgstDataSet.newRecord();
    		}
		 }

	    <%--/*******************************************************************************
	     * FUNCTION 명 : fncInsertModalityInfo (표준양식 저장)
	     * FUNCTION 기능설명 : 표준양식 저장
	     *******************************************************************************/--%>
	    fncInsertModalityInfo = function(){
	    	var pageMode = '${inputData.pageMode}';
	    	console.log('fncInsertModalityInfo pageMode='+pageMode);
	    	...
	    	modalityRgstDataSet.setNameValue(0, 'sbcNm', document.aform.Wec.bodyValue);
	    	...
	    	if(confirm("저장하시겠습니까?")){
		    	if(pageMode == 'V'){
		    		// update
		    		dm1.updateDataSet({
		    	        url: "<c:url value='/knld/pub/insertModalityInfo.do'/>",
		    	        dataSets:[modalityRgstDataSet],
		    	        params: {
		    	        	modalityId : document.aform.modalityId.value
	    	...
	    	dm1.on('success', function(e) {
				var resultData = modalityRgstDataSet.getReadData(e);
	            alert(resultData.records[0].rtnMsg);

	  	    	nwinsActSubmit(document.aform, "<c:url value='/knld/pub/retrieveModalityList.do'/>");
   			...
   			<div class="sub-content">
				...
				<div class="LblockButton top">
   				...
   				<table class="table table_txt_right">
   					...
   					<tbody>
   						<tr>
   							<th align="right"><span style="color:red;">* </span>양식명</th>
   							<td colspan="3">
