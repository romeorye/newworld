...
<script type="text/javascript"
	src="<%=ruiPathPlugins%>/ui/grid/LTotalSummary.js"></script>
<script type="text/javascript"
	src="<%=ruiPathPlugins%>/ui/grid/LGridStatusBar.js"></script>
<link rel="stylesheet" type="text/css"
	href="<%=ruiPathPlugins%>/ui/grid/LTotalSummary.css" />
<link rel="stylesheet" type="text/css"
	href="<%=ruiPathPlugins%>/ui/grid/LGridStatusBar.css" />

<script type="text/javascript"
	src="<%=ruiPathPlugins%>/ui/calendar/LMonthCalendar.js"></script>
<script type="text/javascript"
	src="<%=ruiPathPlugins%>/ui/form/LMonthBox.js"></script>
<link rel="stylesheet" type="text/css"
	href="<%=ruiPathPlugins%>/ui/form/LMonthBox.css" />
...
<%-- RESULT DATASET --%>
	resultDataSet = new Rui.data.LJsonDataSet({
				...
				openGrsEvSnDialog = function() {
					grsEvSnDialog.setUrl('/iris/prj/grs/grsEvStdPop.do?tssCd='
							+ dataSet.getNameValue(0, "tssCd") + '&userId='
							+ dataSet.getNameValue(0, "saSabunNew"));
					grsEvSnDialog.show();
				};
						...
						.on(
								...
								function(e) {
									if (roleCheck == "Y") {
										...
										if (dataSet.getNameValue(0, 'bizDptCd') == "07"
												&& (exSabun == "00207887" || exSabun == "00207772")) {
											$("#butImSave").show();
											$("#btnGrsSave").show();
										} else {
											$("#butImSave").hide();
									...
									if (dataSet.getNameValue(0, 'tssScnCd') == "G") {
										...
										strDt = dataSet.getNameValue(0,
												'tssStrtDd');
										endDt = dataSet.getNameValue(0,
												'tssFnhDd');
										...
										if (dataSet.getNameValue(0, 'tssScnCd') == "D") {
											if (dataSet.getNameValue(0,
													'bizDptCd') == "05"
													&& dataSet.getNameValue(0,
															'custSqlt') == "05") {
												$("#grsDev").hide();
											} else {
												strDt = dataSet.getNameValue(0,
									...
									lvAttcFilId = dataSet.getNameValue(0,
											'attcFilId');
									dataSet.setNameValue(0, 'userId',
											'${inputData._userId}');
									getAttachFileList();
				...
				grsEvMType.on('changed', function(e) {
					if (grsEvMType.getValue() == "IN"
							|| grsEvMType.getValue() == "HD") {
					...
					dataSet
							.setNameValue(0, 'grsEvMType', grsEvMType
									.getValue());
				});
				...
				cfrnAtdtCdTxtNm.on('popup', function(e) {
					openUserSearchDialog(setUsersInfo, 10, userIds, null, 700,
							500);
				});
				...
				bind = new Rui.data.LBind({
					...
					bindInfo : [ {
						...
						id : 'nprodSalsPlnY',
						ctrlId : 'nprodSalsPlnY',
						value : 'value'
					}, {
						id : 'nprodSalsPlnY1',
						ctrlId : 'nprodSalsPlnY1',
						value : 'value'
					}, {
						id : 'nprodSalsPlnY2',
						ctrlId : 'nprodSalsPlnY2',
						value : 'value'
					}, {
						id : 'nprodSalsPlnY3',
						ctrlId : 'nprodSalsPlnY3',
						value : 'html'
					}, {
						id : 'nprodSalsPlnY4',
						ctrlId : 'nprodSalsPlnY4',
						value : 'html'
					}, {
						id : 'nprodSalsCurY',
						ctrlId : 'nprodSalsCurY',
						value : 'value'
					}, {
						id : 'nprodSalsCurY1',
						ctrlId : 'nprodSalsCurY1',
						value : 'value'
					}, {
						id : 'nprodSalsCurY2',
						ctrlId : 'nprodSalsCurY2',
						value : 'value'
					}, {
						id : 'bizPrftPlnY',
						ctrlId : 'bizPrftPlnY',
						...
						id : 'bizPrftProCurY',
						ctrlId : 'bizPrftProCurY',
						value : 'value'
					}, {
						id : 'bizPrftProCurY1',
						ctrlId : 'bizPrftProCurY1',
						value : 'value'
					}, {
						id : 'bizPrftProCurY2',
						ctrlId : 'bizPrftProCurY2',
						value : 'value'
					}, {
						id : 'ptcCpsnY',
						ctrlId : 'ptcCpsnY',
	...
	/* [기능] 첨부파일 조회 */
				...
				getAttachFileInfoList = function() {
					...
					for (var i = 0, size = attachFileDataSet.getCount(); i < size; i++) {
						attachFileInfoList.push(attachFileDataSet.getAt(i)
								.clone());
					}
						...
						$('#attchFileView')
								.append(
										$(
												'<a/>',
												{
													href : 'javascript:downloadAttachFile("'
															+ attachFileList[i].data.attcFilId
															+ '", "'
															+ attachFileList[i].data.seq
															+ '")',
													text : attachFileList[i].data.filNm
															+ '('
															+ attachFileList[i].data.filSize
															+ 'byte)'
												})).append('<br/>');
					}
				...
				var valid = new Rui.validate.LValidatorManager({
					validators : [ {
						...
						id : 'cfrnAtdtCdTxtNm',
						validExp : '회의 참석자:true'
					}, {
						id : 'attcFilId',
						validExp : '첨부파일:true'
					}, {
						id : 'commTxt',
						validExp : 'Comment:true'
					}, {
						id : 'grsEvSn',
						validExp : '평가표:true'
					} ]
				...
				var valid1 = new Rui.validate.LValidatorManager({
					validators : [ {
						...
						id : 'bizPrftProY',
						validExp : '영업이익률:true:minNumber=0.01'
					}, {
						id : 'bizPrftProY1',
						validExp : '영업이익률:true:minNumber=0.01'
					}, {
						...
						id : 'bizPrftPlnY',
						validExp : '영업이익:true:minNumber=0.01'
					}, {
						id : 'bizPrftPlnY1',
						validExp : '영업이익:true:minNumber=0.01'
					}, {
						...
						id : 'nprodSalsPlnY',
						validExp : '매출액:true:minNumber=0.01'
					}, {
						id : 'nprodSalsPlnY1',
						validExp : '매출액:true:minNumber=0.01'
					}, {
						...
						id : 'ptcCpsnY',
						validExp : '투입인원:true:minNumber=0'
					}, {
						id : 'expArslY',
						validExp : '투입비용:true:minNumber=0'
					} ]
				});
				...
				var valid5 = new Rui.validate.LValidatorManager({
					validators : [ {
						...
						id : 'ptcCpsnY',
						validExp : '투입인원:true:minNumber=1'
					}, {
						id : 'expArslY',
						validExp : '투입비용:true:minNumber=0.1'
					} ]
				});
				...
				fncInputChk = function() {
					if (chkVaild == 2) {
						//인원체크
						if (ptcCpsnY1.getValue() == 0
								|| Rui.isEmpty(ptcCpsnY1.getValue())) {
							alert("2차년도 투입인원을 입력하세요");
							return true;
						}

						// 투입비용
						if (expArslY1.getValue() == 0
								|| Rui.isEmpty(expArslY1.getValue())) {
						...
						if (ptcCpsnY2.getValue() == 0
								|| Rui.isEmpty(ptcCpsnY2.getValue())) {
						...
						if (expArslY2.getValue() == 0
								|| Rui.isEmpty(expArslY2.getValue())) {
						...
						if (ptcCpsnY3.getValue() == 0
								|| Rui.isEmpty(ptcCpsnY3.getValue())) {
						...
						if (expArslY3.getValue() == 0
								|| Rui.isEmpty(expArslY3.getValue())) {
						...
						if (ptcCpsnY4.getValue() == 0
								|| Rui.isEmpty(ptcCpsnY4.getValue())) {
						...
						if (expArslY4.getValue() == 0
								|| Rui.isEmpty(expArslY4.getValue())) {
				...
				fncGrsSave = function() {
					...
					var evPoint = $(".L-grid-cell-inner.L-grid-col-calScr:last")
							.html();
					var grsMsg = "";

					if (dataSet.getNameValue(0, 'tssScnCd') == "G") {
						if (dataSet.getNameValue(0, 'grsEvSt').trim() == 'M'
								&& (grsEvMType.getValue() == "IN" || grsEvMType
										.getValue() == "HD")) {
							if (valid.validateGroup('aform') == false) {
								alert(Rui.getMessageManager().get(
										'$.base.msg052')
										+ '\n'
							...
							if (dataSet.getNameValue(0, 'tssType') == "TT") {
							...
							} else if (dataSet.getNameValue(0, 'bizDptCd') == "01"
									|| dataSet.getNameValue(0, 'bizDptCd') == "03"
									|| dataSet.getNameValue(0, 'bizDptCd') == "06") {
								...
								if (valid3.validateGroup('aform') == false) {
									...
									&& dataSet.getNameValue(0, 'custSqlt') == "05") {
								if (valid4.validateGroup('aform') == false) {
									alert(Rui.getMessageManager().get(
											'$.base.msg052')
											+ '\n'
						...
						if (dataSet.getNameValue(0, 'tssType') == "CB"
								|| dataSet.getNameValue(0, 'tssType') == "IA"
								|| dataSet.getNameValue(0, 'tssType') == "IB"
								|| dataSet.getNameValue(0, 'tssType') == "IC") {
						...
						if (dataSet.getNameValue(0, 'bizDptCd') == "05"
								&& dataSet.getNameValue(0, 'custSqlt') == "05") {
						...
						if (dataSet.getNameValue(0, 'tssScnCd') == "D") {
							...
							if (dataSet.getNameValue(0, 'tssType') == "CB"
									|| dataSet.getNameValue(0, 'tssType') == "BB"
									|| dataSet.getNameValue(0, 'tssType') == "IA"
									|| dataSet.getNameValue(0, 'tssType') == "IB"
									|| dataSet.getNameValue(0, 'tssType') == "IC") {
					...
					if (dataSet.getNameValue(0, 'grsEvSt') == "M"
							&& Rui.isEmpty(grsEvMType.getValue())) {
					...
					if (dataSet.getNameValue(0, 'tssType') == "CB"
							|| dataSet.getNameValue(0, 'tssType') == "BB"
							|| dataSet.getNameValue(0, 'tssType') == "TT"
							|| dataSet.getNameValue(0, 'custSqlt') == "05"
							|| dataSet.getNameValue(0, 'tssType') == "IA"
							|| dataSet.getNameValue(0, 'tssType') == "IB"
							|| dataSet.getNameValue(0, 'tssType') == "IC") {
					...
					Rui
							.confirm({
								text : grsMsg
										+ '평가완료하시겠습니까?<br>완료후에는 수정/삭제가 불가능합니다.',
								handlerYes : function() {
									dm
											.updateDataSet({
												modifiedOnly : false,
				...
				fnAttchValid = function() {
					...
					if (!Rui.isEmpty(tmpAttchFileList)) {
						for (var i = 0; i < tmpAttchFileList.length; i++) {
							if (tmpAttchFileList[i].data.filNm.indexOf('통합심의서') > -1
									|| tmpAttchFileList[i].data.filNm
											.indexOf('통합 심의서') > -1
									|| tmpAttchFileList[i].data.filNm
											.indexOf('회의록') > -1
									|| tmpAttchFileList[i].data.filNm
	...
	<div class="contents">
		<div class="sub-content">
			...
			<form id="pform" name="pform" method="post">
				<input type="hidden" id="attcFilId" name="attcFilId" /> <input
				...
				<table class="table table_txt_right">
					...
					<tbody>
						...
						<tr>
							...
							<td class="tssLableCss"><span id="tssAttrNm"></<span></td>
							<th align="right">과제명</th>
							<td><span id="tssNm"></span></td>
						</tr>
						<tr>
							<th align="right">심의단계</th>
							<td class="tssLableCss"><span id="grsEvStNm"></<span>
										<!-- <span id="grsEvMTypeNm"></<span> --></td>
						...
						<tr>
							...
							<td><button type="button" class="btn" id="attchFileMngBtn"
									name="attchFileMngBtn"
									onclick="openAttachFileDialog(setAttachFileInfo, getAttachFileId(), 'prjPolicy', '*')">첨부파일등록</button></td>
						</tr>
						<tr id="trGrsEvMType" style="display: none;">
							<th align="right"><span style="color: red;">* </span>중간평가 방법
								선택</th>
				...
				<table class="table table_txt_right" id="grsDev">
					...
					<tbody>
						<tr id="trCtyOtPlnM" style="display: none;">
							<th>상품출시계획</th>
							<td colspan="5"><input type="text" id="ctyOtPlnM" /></td>
						</tr>
						<tr id="trAncpOtPlnDt" style="display: none;">
							<th>예상출시일(계획)</th>
							<td colspan="5"><input type="text" id="ancpOtPlnDt" /></td>
						</tr>
