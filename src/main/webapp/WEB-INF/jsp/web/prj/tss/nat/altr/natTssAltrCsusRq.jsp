...
//                     fnCsusContentsCreate("SAVE");
        ...
        fnCsusContentsCreate = function(gbn) {
            ...
            var altrRsonTxt = stringNullChk(resultJson.jsonSmry.altrRsonTxt) == "" ? "" : resultJson.jsonSmry.altrRsonTxt.replace(/\n/g, "<p>"); //변경사유            
            var addRsonTxt  = stringNullChk(resultJson.jsonSmry.addRsonTxt)  == "" ? "" : resultJson.jsonSmry.addRsonTxt.replace(/\n/g, "<p>");  //추가사유
            
            csusCont = "";
            csusCont += '<div class="titleArea"><h2>${resultMst.tssNm}</h2></div>';
            csusCont += '<br/>';
            csusCont += '<div class="titArea alignR">';
            csusCont += '            과제리더: ${resultMst.saUserName}, 작성일: ${resultInfo.createDate}</h4>';
            csusCont += '</div>';
            ...
            csusCont += '                    <tr>';
            csusCont += '                        <td>${resultAltr.prvs}</td>';
            csusCont += '                        <td>${resultAltr.altrPre}</td>';
            csusCont += '                        <td>${resultAltr.altrAft}</td>';
            csusCont += '                    </tr>';
            </c:forEach>
            csusCont += '                </tbody>';
            csusCont += '            </table>';
