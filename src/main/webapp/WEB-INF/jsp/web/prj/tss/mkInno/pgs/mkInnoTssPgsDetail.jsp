...
//============================yldDataSet ================================================================//
		...
		/* 조회*/
        dm.loadDataSet({
            //dataSets: [mstDataSet, smryDataSet, mbrDataSet, yldDataSet],
...
/*******************버튼이벤트 ***************************************************************************************/
           ...
           fnMbrSave = function(){
            ...
            fncCmpl = function(){
            	...
            	var dm1 = new Rui.data.LDataSetManager();
          	
            	dm1.on('load', function(e) {
        		});
            	...
            	dm1.on('success', function(e) {
        			var data = resultDataSet.getReadData(e);
       				...
       				if( data.records[0].rtnSt == "S"){
       					document.aform.tssCd.value = data.records[0].cmplTssCd;
       					nwinsActSubmit(document.aform, "<c:url value='/prj/tss/mkInno/mkInnoTssCmplDetail.do'/>");
       				}
                });
            	
          		if(confirm("과제 완료를 하시겠습니까?")) {
            		 dm1.updateDataSet({
                         url:'<c:url value="/prj/tss/mkInno/saveMkInnoTssCmpl.do"/>',
                         dataSets:[mstDataSet],
                         params: {
                         	tssCd: tssCd
                           ,pkWbsCd :mstDataSet.getNameValue(0,'pkWbsCd')
                         }
                     });
            	}            	
            }
	...
	<div id="mstFormDiv">
		<div class="titArea mt0">
        	<div class="titArea">
                ...
                <button type="button" id="btnCsusRq" name="btnCsusRq" onClick="fncCmpl();">완료진행</button>
